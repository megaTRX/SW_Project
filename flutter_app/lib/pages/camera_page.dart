...
class _CameraPageState extends State<CameraPage> {
  ...
  String _getInactivityStatus() {
    ...
    return unresolved.isEmpty ? '정상' : '비활동 감지';
  }
            ...
            child: Column(
              ...
              children: [
                    ...
                    GestureDetector(
                      ...
                      child: _isLoading
                          ...
                          : Row(children: const [
                              ...
                              SizedBox(width: 4),
                              Text('새로고침', style: TextStyle(fontSize: 13, color: Color(0xFF6366F1), fontWeight: FontWeight.w500)),
                            ]),
                    ),
                  ],
                ),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      _AIRow(label: '마지막 움직임 감지', value: _getLastMotionText(), icon: Icons.timer_rounded, color: const Color(0xFF10B981)),
                      const Divider(height: 20, color: Color(0xFFE2E8F0)),
                      _AIRow(label: '비활동 감지', value: _getInactivityStatus(), icon: Icons.accessibility_new_rounded, color: _getInactivityColor()),
                      const Divider(height: 20, color: Color(0xFFE2E8F0)),
                ...
                Row(
                  ...
                  children: [
                    ...
                    GestureDetector(
                        ...
                        showModalBottomSheet(
                          ...
                          builder: (_) => DraggableScrollableSheet(
                            ...
                            builder: (_, controller) => ListView(
                              ...
                              padding: const EdgeInsets.all(16),
                              children: [
                                const Text('전체 감지 기록', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                                const SizedBox(height: 12),
                      ...
                      child: const Text('전체 보기', style: TextStyle(fontSize: 13, color: Color(0xFF6366F1), fontWeight: FontWeight.w500)),
                    ),
                  ],
                ),
                const SizedBox(height: 12),
                if (_isLoading)
                  const Center(child: Padding(padding: EdgeInsets.all(30), child: CircularProgressIndicator(color: Color(0xFF6366F1))))
                  ...
                  Container(
                    ...
                    child: const Center(child: Column(children: [
                      ...
                      SizedBox(height: 8),
                      Text('최근 감지 기록이 없어요', style: TextStyle(color: Color(0xFF94A3B8), fontSize: 14)),
                    ])),
                  )
                else
                  ..._alerts.take(5).map((alert) {
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: isAlert ? const Color(0xFFFECACA) : const Color(0xFFFDE68A)),
                      ),
                      child: Row(children: [
                        Container(
                          width: 38, height: 38,
                          decoration: BoxDecoration(
                            color: isAlert ? const Color(0xFFFEF2F2) : const Color(0xFFFFFBEB),
                            borderRadius: BorderRadius.circular(10),
                          ),
                          child: Icon(isAlert ? Icons.emergency_rounded : Icons.warning_rounded,
                              color: isAlert ? const Color(0xFFEF4444) : const Color(0xFFF59E0B), size: 20),
                        ),
                        const SizedBox(width: 12),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(alert["content"] as String, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF1E293B))),
                          const SizedBox(height: 2),
                          Text(alert["time"] as String, style: const TextStyle(fontSize: 12, color: Color(0xFF94A3B8))),
                        ])),
                        Container(
                          ...
                          child: Text(alert["status"] as String,
...
class _DetectCard extends StatelessWidget {
  ...
  const _DetectCard({required this.icon, required this.iconBg, required this.iconColor, required this.label, required this.value, required this.valueColor});
